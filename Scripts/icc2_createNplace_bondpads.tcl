...
# Example:                                                          #
#   createNplace_bondpads -inline_pad_ref_name BONDINNER ;#inline     #
...
proc createNplace_bondpads {args} {
  ...
  if {[info exists pargs(-inline_pad_ref_name)]} {
   ...
   set all_left_io_cell_list [collection_to_list -name_only -no_braces [get_cells -hierarchical -filter "pad_cell==true && orientation==R270"]]
   foreach left_io_cell $all_left_io_cell_list {
       set cellname [get_object_name [get_lib_cells -of_objects $left_io_cell]]
       if [ regexp {.*CORNER.*} $cellname  ] {
           continue
       }
   	   set io_sort_index [lindex [get_attribute [get_cells -hierarchical $left_io_cell] origin] 1]
   ...
   set all_top_io_cell_list [collection_to_list -name_only -no_braces [get_cells -hierarchical -filter "pad_cell==true && orientation==R180"]]
   foreach top_io_cell $all_top_io_cell_list {
       set cellname [get_object_name [get_lib_cells -of_objects $top_io_cell]]
       if [ regexp {.*CORNER.*} $cellname  ] {
           continue
       }
   	   set io_sort_index [lindex [get_attribute [get_cells -hierarchical $top_io_cell] origin] 0]
   ...
   set all_right_io_cell_list [collection_to_list -name_only -no_braces [get_cells -hierarchical -filter "pad_cell==true && orientation==R90"]]
   foreach right_io_cell $all_right_io_cell_list {
       set cellname [get_object_name [get_lib_cells -of_objects $right_io_cell]]
       if [ regexp {.*CORNER.*} $cellname  ] {
           continue
       }
   	   set io_sort_index [lindex [get_attribute [get_cells -hierarchical $right_io_cell] origin] 1]
   ...
   set all_bottom_io_cell_list [collection_to_list -name_only -no_braces [get_cells -hierarchical -filter "pad_cell==true && orientation==R0" ]]
   foreach bottom_io_cell $all_bottom_io_cell_list {
       set cellname [get_object_name [get_lib_cells -of_objects $bottom_io_cell]]
       if [ regexp {.*CORNER.*} $cellname  ] {
           continue
       }
   	   set io_sort_index [lindex [get_attribute [get_cells -hierarchical $bottom_io_cell] origin] 0]
   ...
   if { $exist_bond_pad_list !=""} {
      ...
      echo ">>>> current cell don't exist inline bond pad cell $bond_pad_ref_name."      
      }

   ## remove current exist stagger bonding pad cell
   if {$stagger == "true"}  {
     set get_stagger_bond_pad_cells_cmd "get_cells -hierarchical -filter \"ref_name =="
     ...
     if { $exist_stagger_bond_pad_list !=""} {
        ...
        echo ">>>> current cell don't exist stagger bond pad cell $stagger_bond_pad_ref_name."      
	}
   }
   ...
   foreach io_cell $all_io_cell_list {
      ...
      set io_cell_bbox [get_attribute [get_cells -hierarchical [lindex $io_cell 0]] bbox]
      set io_cell_orient [get_attribute [get_cells -hierarchical [lindex $io_cell 0]] orientation]
      ...
      set bond_pad_name ""
      append bond_pad_name [get_attribute [get_cells -hierarchical [lindex $io_cell 0]] name] "_PAD"
...
puts "move_objects -x $bond_pad_LL_X -y $bond_pad_LL_Y [get_cells $bond_pad_name]"
   
    move_objects -x $bond_pad_LL_X -y $bond_pad_LL_Y [get_cells $bond_pad_name]
